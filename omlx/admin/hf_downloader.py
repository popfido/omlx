...
def _get_hf_api() -> tuple[HfApi, str | None]:
    ...
    Resolution order:
      1. omlx setting `huggingface.endpoint` (admin UI)
      2. `HF_ENDPOINT` environment variable (huggingface_hub default)

    Whichever wins is run through `_resolve_endpoint()` to follow permanent
    cross-origin redirects (e.g. hf-mirror.com → huggingface.co from non-CN
    IPs) so downstream HF library code sees a stable origin.
